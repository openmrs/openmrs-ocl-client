const buildAddConceptToDictionaryMessage = (results: {expression: string, added: boolean}[]) => {
  ...
  const alreadyAddedMessage = alreadyInDictionaryCount > 0 ? ` ${alreadyInDictionaryCount} already added concept${wasOrWere(alreadyInDictionaryCount)} skipped` : '';
