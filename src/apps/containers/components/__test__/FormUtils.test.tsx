...
import {testAPIOrgList, testProfile} from "../../../authentication/__test__/test_data";
...
describe('showOrganisationHeader', () => {
    ...
    it('should return Your Organisations Sub header if user is part of any organisation', () => {
        // @ts-ignore
        const organisationsSubHeader = showOrganisationHeader(testAPIOrgList).props.children
...
describe('showUserOrganisations', () => {
    ...
    it('should return all organisations user is part of', () => {
        const userOrganisations: JSX.Element[] | null = showUserOrganisations(testAPIOrgList);

        let expectedOrganisations: Array<String> = [];
        testAPIOrgList.forEach((userOrg) =>
            expectedOrganisations.push(userOrg.name)
        );
