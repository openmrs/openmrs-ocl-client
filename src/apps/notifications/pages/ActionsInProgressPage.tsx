...
const ActionsInProgressPage: React.FC<Props> = ({
  ...
  const inProgressListLocalStorage = getLocalStorageObject({
    name: "notification",
    key: "inProgressList",
    value: inProgressList
  });

  const loadingListLocalStorage = getLocalStorageObject({
    name: "notification",
    key: "loadingList",
    value: loadingList
  });

  const erroredListLocalStorage = getLocalStorageObject({
    name: "notification",
    key: "erroredList",
    value: erroredList
  });

  const successListLocalStorage = getLocalStorageObject({
    name: "notification",
    key: "successList",
    value: successList
  });
  ...
  const successfulItems = loadingListLocalStorage
    ...
    .filter(
      (notification: NotificationItem) => notification && notification.progress
    )
    ...
    <Header title="Progress Notifications">
      <Grid item xs={6}>
        ...
        {!inProgressItems.length || !importMetaDataItems.length ? null : (
          <List
            subheader={<Chip label="In Progress" icon={<AutorenewIcon />} />}
          >
        ...
        {!erroredItems.length || !importMetaDataItems.length ? null : (
          <List subheader={<Chip label="Failed" icon={<ErrorIcon />} />}>
        ...
        {!successfulItems.length || !importMetaDataItems.length ? null : (
          <List
            subheader={<Chip label="Completed" icon={<CheckCircleIcon />} />}
          >
            {successfulItems.map((notification, index) => (
              <NotificationCard
                key={index}
                headerMessage={notification.progress.split(SEPARATOR)[0]}
                subHeaderMessage={buildAddConceptToDictionaryMessage(
                  notification.result
                )}
                importMetaData={importMetaDataItems[index]}
