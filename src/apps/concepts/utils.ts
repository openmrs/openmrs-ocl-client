...
export function canModifyConcept(conceptUrl: string, profile?: APIProfile, usersOrgs: APIOrg[] = []) {
  const CONCEPT_PATTERN = '/:ownerType/:owner/(collections/:collectionId)(sources/:sourceId)/concepts/:conceptId/';

  const matches = getParams(CONCEPT_PATTERN, conceptUrl);
  if (!matches) return false;

  if (matches.ownerType === USER_TYPE) return profile?.username === matches.owner;
