...
const api = {
  retrievePublicSources: (page: number=1, limit: number=10, q='') => authenticatedInstance.get('/sources/', {params: {page, limit, q: buildPartialSearchQuery(q), verbose: true, timestamp: new Date().getTime()}}),
  concepts: {
    create: (sourceUrl: string, data: BaseConcept): Promise<AxiosResponse<any>> => authenticatedInstance.post(`${sourceUrl}concepts/`, data),
    retrieve: (sourceUrl: string, page: number=1, limit: number=10, q='', sortDirection='sortAsc', sortBy='bestMatch', dataTypeFilters=[], classFilters=[]) => authenticatedInstance.get(`${sourceUrl}/concepts/`, {params: {page, limit, q: buildPartialSearchQuery(q), [sortDirection]: sortBy, ...optionallyIncludeList('datatype', dataTypeFilters), ...optionallyIncludeList('conceptClass', classFilters), timestamp: new Date().getTime()}}),
  },
  concept: {
