...
const api = {
  ...
  concepts: {
    ...
    retrieve: (conceptsUrl: string, page: number=1, limit: number=10, q='', sortDirection='sortAsc', sortBy='bestMatch', dataTypeFilters=[], classFilters=[]) => authenticatedInstance.get(conceptsUrl, {params: {page, limit, q: buildPartialSearchQuery(q), [sortDirection]: sortBy, ...optionallyIncludeList('datatype', dataTypeFilters), ...optionallyIncludeList('conceptClass', classFilters), timestamp: new Date().getTime()}}),
  },
  concept: {
