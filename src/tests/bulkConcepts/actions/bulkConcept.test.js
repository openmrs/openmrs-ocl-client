...
describe('Test suite for addBulkConcepts async actions', () => {
  ...
  it('should add concept on ADD_EXISTING_CONCEPTS action dispatch', () => {
    ...
    return store.dispatch(addConcept(params, {data: {expressions: ['test']}}, 'lob dev')).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
      console.log(store.getActions(),'the action');
      expect(notifyMock).toHaveBeenCalledTimes(1);
...
describe('recursivelyFetchConceptMappings', () => {
  ...
  it('returns a single level of mappings when called with 0 levelsToCheck', async () => {
    ...
    expect(fetchFromPublicSourcesMock).toHaveBeenCalledWith(conceptCode);
    expect(result).toEqual([mappings[0].to_concept_url]);
  });

  it('returns two levels of mappings when called with 1 levelsToCheck', async () => {
    fetchFromPublicSourcesMock.mockClear();
    ...
    expect(fetchFromPublicSourcesMock.mock.calls[0][0]).toEqual(conceptCode);
  ...
  it('does not return duplicate concept references', async () => {
    fetchFromPublicSourcesMock = jest.fn()
      .mockReturnValueOnce({ data: [mappings[0]] })
      .mockReturnValueOnce({ data: [mappings[0]] });
    api.mappings.fetchFromPublicSources = fetchFromPublicSourcesMock;
