...
const loadingSelector = (...actions: IndexedAction[]) => (
  state: AppState
): boolean =>
  actions.reduce(
    (
...
const progressSelector = ({ actionType, actionIndex }: IndexedAction) => (
  state: AppState
): any =>
  state.status[progress(actionType)]
    ? state.status[progress(actionType)][actionIndex]
    : undefined;
const errorSelector = ({ actionType, actionIndex }: IndexedAction) => (
  state: AppState
): any =>
